...
| receipt_document_id | INTEGER | FK → Family Documents (SET NULL on delete), nullable; the visit API returns the document's name as `receipt_document_name` only to callers who may read that document (Documents access, token scope, document visibility, #1358), otherwise `null` |
