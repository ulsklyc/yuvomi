...
**The today sheet speaks for every module that has something today (2026-09-23 critique · #1449):** the program above read only appointments, tasks, meals and shopping and closed with "Nothing else today" while the same `/dashboard` answer carried open doses and a pending approval. The further sources are one list in `public/utils/today-sheet.js` (`TODAY_SHEET_SOURCES`), each turning one payload field into rows of one shape (kind, sort key, tone, seal, title, route, priority, `open`): open doses of the viewer's own medications (`health`), pending reward approvals (only when `rewards.view === 'approver'` - a child's `pending` counts its own requests), today's bin and tomorrow's "put it out tonight" (`wastePickups`, today and tomorrow, Waste module; today's pickup counts as open until 12:00), birthdays and name days today (not when the calendar already carries them), the viewer's own shift today (`myShiftsToday`, Schedule module; free days are no row), due reminders (`GET /reminders/pending`, except those whose object is already a row), the housekeeper on site, and pantry batches that expire today (`pantryExpiring.todayItems`/`todayCount`, one row, opening `/pantry?filter=soon`). Every source passes the same three gates (`sourceSpeaks()`): module switched on and not `none`, widget not locked for the viewer, and its own widget **not** visible in the grid (the echo rule). The cap keeps open rows before informational ones; the rows then show in time order. **"Nothing else today" only falls when it is true:** there are rows, none is open and none hides behind "+N more". Finished appointments leave the sheet at their end time (read in the household zone, `eventHasEnded()`), without a reload - the sheet rebuilds on the minute. From a sheet width of 720px the rows run in two columns. The wall display reads the same model. **The number of pending approvals stands in exactly one place:** the rewards widget when it is visible, otherwise the today sheet, and only when the sheet is hidden the parents' metric tile.
...
- Upcoming events: next 3–5, color-coded by person; each row navigates to `/calendar?open=<id>&date=YYYY-MM-DD` so the event detail popup opens on the displayed occurrence, including recurring series instances. **Since the 2026-09-23 critique (#1449)** `upcomingEvents` also carries the appointments of today that have already ended (end before now in the household zone, never all-day ones, `keepEndedToday` in `getUpcomingEvents()`); the cap of five counts only what is still coming. The tile shows the two latest finished ones stepped back above the coming ones ("Over", `event-item--ended`) and folds the rest into "+N earlier today". It carries **no count badge**: the list is open-ended and cut at five, so "5" only ever named its own cap. **At 2x1 the tile is the week ahead** (`public/utils/week-strip.js`): seven days from today, a dot per event in the colour of the event, person or calendar (at most six per day, then "+n"), up to two band lanes for all-day and multi-day events with an open end where they continue, today marked with `aria-current="date"`. Its data is `weekEvents` (events touching the household days from yesterday to a week ahead, compact shape, same visibility, `events_scope=mine` and `events_birthdays=hide` as the list; empty when Calendar is denied). Each day is a link to `/calendar?date=YYYY-MM-DD`. Day assignment of a synced event (`...Z` or with offset) follows the household zone (`zonedDateKey`), not the device
...
- Budget: the monthly balance as a stacked readout (label above, 28px figure below - it used to be a 22px amount at the right end of a caption line, built like the supporting metric under it), the savings rate with its track, and a quiet income/expenses line. The tall size (1x2) lists the three largest expense categories of the month (`budget.topExpenses`)
- Family members: since v2.4.0 a per-member "today" card — avatar, name, and what today holds (next own appointment, open-task count from `memberTodayTasks`); on free days the member's next upcoming appointment. Since v2.6.0 a footer closes the card with the household's task tally for the day ("Tasks today: 3 open, 2 done", or "No tasks assigned for today" when both are zero). Not offered in a household of one. The "Manage" header link leads to Settings while the seal deliberately speaks the contacts tone (people, not administration). **Since the 2026-09-23 critique (#1449)** a member's row shows the **next** appointment that has not ended (the same `eventHasEnded()` rule as the event tile, household zone; a running one still counts, an all-day one never ends on its own day, a timed one goes before an all-day one), "Done for today" after the last one, and "Only shared appointments" when that is all today holds. An appointment for two or more of the shown members appears **once**, in its own row above the members ("Everyone" or the names). Still open from #1449: the card borrows the event tile's list (five entries, its "only mine" filter)
- Rewards (v0.96.0, reworked after the 2026-09-23 critique): **progress, not a leaderboard** - no ranks, no highlighted leader. The view follows the role the module itself asks for approving (`isAdminRequest`), not `family_role`, and the server sends it as `rewards.view`: **self** (participates, cannot approve) gets only the own row - balance, a quiet bar toward the next reward (`public/utils/reward-goal.js`, the same rule as the module's overview), the last three earned or bonus points and the own pending requests; the siblings are not in the payload at all. **approver** sees every participant side by side, ordered by name, each with balance and goal, plus "N to approve" linking to `/rewards`. **family** (display tablet, members who neither collect nor approve) sees the same without pending requests. A single participant who is the viewer gets the self view. The metric tile shows the own balance and goal, or pending approvals for an approver, never a leader
...
- Pantry "Expiring soon" (`pantry`): pantry batches whose best-before date has passed or falls within `EXPIRY_SOON_DAYS` (7, the same threshold as the pantry's own "expiring soon" chip), expired first, then by days left. One row per batch (`pantry_items` row) - no grouping by product until #1448. Batches with quantity 0 are left out. Served in the `/dashboard` payload as `pantryExpiring: { items, todayItems, total, expiredCount, todayCount }` (`server/services/pantry-expiring.js`); `items` is capped at 5 and carries `days_left` computed from the household day, the counts are uncapped, and `todayItems` lists what expires today on its own because those rows can fall behind expired ones in the capped list. Module `pantry`: `none` or a token without a pantry scope gets the empty shape; a failed query gives `null` and the tile's retry state. Display only; rows open `/pantry?filter=expired|soon`, which the pantry page reads as its filter chip. Default-hidden. What expires **today** also speaks in the today sheet while the tile is not visible (see the today sheet above)
...
**Views:** Month (default on desktop; on a phone the grid plus a list of the selected day), Week (hour grid), Day (timeline), Agenda (list). On mobile the first load defaults to Agenda view; after the user manually switches views the selected view is persisted for subsequent visits.

**Choosing a view persists it, drilling into one does not (v2.51.0).** Clicking a day cell on a wider screen opens the day view for that date (on a phone the tap selects the day instead, see below), but leaves the stored default alone - `setSavedCalendarView()` belongs in the tablist's `onChange`, where the user *chooses*, and nowhere else. Until v2.51.0 the drill-in wrote it too, so a navigation gesture silently changed a setting: three taps on a month cell and the calendar opened in day view from then on, with no feedback and no way back other than noticing it.

**A day link opens that day (2026-09-23):** `/calendar?date=YYYY-MM-DD` without `open` opens the day view of that date and, like the drill-in, does not store the view (`dayDeepLinkDate()`). The week strip of the dashboard's calendar tile links its days this way; with `open=<id>` the link keeps opening the event as before.
...
**The header on a phone (2026-09-24 critique, P1).** The header carries the page title, the period row (back, label, forward, "Today") and the bar row; filter and search sit at the end of the bar row, after the view switcher, in every view and at every width - in the actions slot they used to build a header row of their own on a phone (56px) and stood left or right depending on the collapse state. When the week, day or agenda is scrolled, the title row folds away completely below 1024px (the `<h1>` stays in the accessibility tree, clipped) and the period row stays: 166px expanded, 121px collapsed at 375px, against 230/216px before. A view switch lets the shell re-judge the collapse against the new scrollport, so a month that cannot scroll does not inherit the collapsed head of the week. Only a scroll the user drives folds the title (2026-09-25 re-critique): the shell remembers the target of the last gesture (wheel, touch, pointer, key) and ignores scrolls of a port the gesture was not in, so the week's jump to the current hour and the re-judge after a switch never fold it - the view tabs stay where the finger left them. A scroll without a gesture may only unfold a head the new port cannot carry. The period label has a fixed width - the rest of the row on a phone, 15rem from 768px - so the arrows do not move between views; on a phone the day view names the weekday short ("Do, 24.09.2026"). The week's day headings put weekday and date in one line (44px), and below 640px the hour column is 44px (`--cal-gutter-width-compact`) with 12-hour labels shortened to "8 AM".

**Swiping between periods (2026-09-24).** On a touch screen a horizontal swipe over the month, week or day view moves one period forward or back (`public/utils/period-swipe.js`, wired on `#cal-body`); the agenda and the search results have no neighbouring period and take no swipe. The gesture uses the list-swipe thresholds (80px, 12px direction lock, haptic at the threshold): a mainly vertical drag scrolls and ends the gesture for that touch, a touch starting within 20px of the screen edge belongs to the system back gesture, and an open dialog takes the gesture from the surface below. The time direction follows the reading direction (in `ar`/`fa` a swipe to the right moves forward). The content follows the finger and the new period slides in from the side it came from; with `prefers-reduced-motion` it does neither. `#cal-body` carries `touch-action: pan-y pinch-zoom`. The arrow buttons remain the path for mouse and keyboard.

**The arrows say what they do (2026-09-24).** The back and forward buttons are named after their step - "Previous month", "Next week", "Next 3 days" (the phone's three-day week), "Next 30 days" (agenda), "Previous week" in the collapsed phone month - as accessible name and tooltip; they used to be "Back"/"Forward" in every view, and "Forward" in the agenda silently moved thirty days. The agenda's period label names its span ("24.09. - 24.10.2026", the range `getAgendaRange()` loads) instead of only its start ("From 24.09.2026").

**Keyboard and screen reader (2026-09-24).** The month grid is one ARIA grid with a single tab stop (`role=grid` on `.month-view`, the weekday strip as column headers, one `role=row` per week, each day a `gridcell` with a roving tabindex). Arrow keys move the day and change the month at the edge, Home/End go to the start/end of the week (household week start), Page Up/Down move one month with the day clamped. Enter/Space does what a tap does: on a phone it selects the day (`aria-selected`) and a polite live region outside `#cal-body` announces "Friday, 25.09.2026, 2 entries" - on a selection only, never on a re-render; on a wider screen it opens the day view and moves focus to the day's first entry. Events inside a month cell are not tab stops of their own: the path to them goes through the cell into the day view, where every entry is a button (an in-cell interaction mode would be undiscoverable, and a cell only shows what fits). The cell's name carries weekday, date, "today", the count and the first three titles. In week and day view every event block, all-day bar and week day heading is a button (`role=button`, `tabindex=0`) named like the agenda row (series, title, time, place, calendar, people); within a column the DOM order - and so the tab order - is the start time, shift blocks included. Enter/Space opens what a click opens. A task's accessible name includes its priority and time. The view tablist is named "View". **Shortcuts on `/calendar`** (after Google Calendar): `t` today, `k`/`j` previous/next period (the arrow keys too while no control has focus, mirrored in RTL), `m`/`w`/`d`/`a` month/week/day/agenda. They live in the shell's single `SHORTCUTS` dispatcher with `route: '/calendar'`, so the `g d` chord never also means `d`; the help dialog (`?`) lists them only on that route, and the buttons carry `aria-keyshortcuts`. **The event sheet puts "Edit" at the bottom:** on a phone the read view's footer ends with a primary "Edit" button in thumb reach while "Delete" stays a danger-ghost button at the start (`openDetailView({ edit: { primary: true } })`); the header button appears only in the form, as "Back".

**One event grammar (2026-09-24 critique, P2).** An event looks the same in every view: a tinted surface in its colour, a 3px solid edge at the inline start (`--cal-event-edge`), one ink recipe (35 % event colour in the primary text colour), the title at the top of a timed block and the time as a second line only when the block is tall enough (a container query on the block's height, since an hour is 56px in the week and 40px in the day). Agenda rows and the phone month's day list keep the list surface but carry the same edge beside their text instead of an 8px dot. The day view's separate spine element is gone. **Icons:** an event shows its icon only when one was chosen (`hasEventIcon()` - the default `calendar` glyph adds nothing inside the calendar), and the repeat mark on every series, in every view through one builder (`eventGlyphsHtml()`), drawn in the title's ink rather than the raw event colour; the phone month's title mode leaves them out for lack of room. **Time:** every span goes through `timeSpanText()` - "17:00 - 18:30", the separator from `calendar.dayRangeLabel`, the times from `formatTime()` (12-hour aware, shift times included), the locale suffix ("Uhr") once at the end. The grid omits the suffix; lists, the event and shift details, tooltips and accessible names carry it. The detail's "When" row reads "24.09.2026 10:00 - 11:30 Uhr" instead of "10:00 Uhr - 11:30 Uhr". The hour column's all-day label is a full word with a soft hyphen in German ("ganz-/tägig") and wraps inside the 44px column.

**Multi-day events are one band per week row (2026-09-25 re-critique, P2).** An event that touches several days and belongs in the all-day row (all-day, or 24 hours and longer; `isBandEvent()`, the same rule as the dashboard week strip) is drawn as a single bar across its days in the week's all-day row and in the month from 640px, not as a chip on every day. Lanes are packed by `packLanes()` from `public/utils/week-strip.js`, shared with the dashboard tile: ordered by the real start, then the longer event first, each into the lowest free lane; single-day entries sit below the lanes. The bar keeps the event grammar; its colour edge and rounding appear only at the event's real start and end, and where it runs past the row the end is open (no edge, no rounding, up to the column border) with a chevron (`.cal-band--before`/`--after`), mirrored in RTL. In the week the bar is a button like every block; its title, "from" time and assignees stand once at its start, the "until" time at its end, and whatever does not fit next to the whole title drops out. In the month the bars lie in a layer over the seven cells (`.month-bands`, `aria-hidden`), each cell keeps its occupied lanes free (`.month-day__lanes`), and `fitMonthDayCells()` decides per row how many lanes stay visible; hidden bars count into the cell's "+N". Over days of the previous or next month a bar takes the same paler tint as the chips there (`.cal-band--outside`), and the focus ring of a month cell is drawn above the layer. The cell stays the only tab stop and names a band as "Title (Day 2 of 3)"; a cell that names three of four entries adds "and 1 more" (`calendar.monthDayMoreTitles`). A bar's accessible name gives its dates ("13 to 15 October", `spokenDateSpan()` over `Intl.DateTimeFormat#formatRangeToParts` with the locale's word instead of its dash) and "continued" for a piece that starts in an earlier week. The phone month keeps a dot per day; the agenda and the day list keep one row per day and add "Day 2 of 3" (`calendar.multiDayPosition`) after the time; the day view shows the day's piece with open ends. Short overnight events (#1313) are not bands.

**The month on a phone (2026-09-24 critique, P2).** Below 640px the month is split: the grid on top says *where* something is, a list of the selected day below says *what* (the reference is Apple Calendar's list month, Fantastical, Outlook). A tap on a day **selects** it instead of opening the day view; `state.cursor` is the selected day, marked by a 2px ring around the numeral (today keeps its filled accent circle, a selected today shows both). A day of the neighbouring month pages there and selects it. The list header shows the date and weekday with a chevron and opens the day view; the list itself reuses the agenda rows (`dayGroupHtml()`), so events keep their agenda row and tasks their checkbox chip, and a free day shows a quiet empty state with an "Add event" action for that day. Paging months, by arrow or swipe, selects today when the new month contains it and otherwise the 1st (`monthStepCursor()`, the same rule as `defaultDateInPeriod()`); "Today" stays visible while another day is selected. Dots only mark presence now: centred, at most four, no "+N"; a task is a small rounded square and an event a round dot, both ringed in `--color-text-tertiary`, so the kind no longer hangs on a ring colour that disappeared in dark mode. Dragging the list up or scrolling it collapses the grid to the week of the selected day, dragging down or scrolling back to the top expands it, and a button in the list header does the same without a gesture (`aria-expanded`); scrolling only collapses when the list can still scroll afterwards, otherwise it would clamp to the top and expand again. No transition under `prefers-reduced-motion`. Collapsed stays for the visit, a new visit starts expanded. **Collapsed, the month moves by week** (2026-09-24): swipe, the arrows and `j`/`k` move the selected day by seven days and the month follows when the selection crosses into the next or previous one, as in Apple Calendar; expanded they keep moving by month. One function decides the step for every view (`periodStepOf()`), and the same answer names the arrows. With "Event titles" on, the week rows share a fixed 320px instead of taking 80px each (80px for four weeks, 64px for five, 53px for six), so the day list below keeps the same height in every month; a cell that runs out of room shows "+2" instead of "+2 more", which wrapped and was cut off at ~50px. Rotating across 640px redraws the month in the other form. **The grid has as many week rows as the month needs (4-6) on every width**, and the load window ends with the last row (`monthGridSpan()`); a fixed 42 days used to draw and load a sixth row that was entirely October in September 2026.

**The event dialog asks the common things first (2026-09-24 critique, P2).** The fields follow how often they are used: title, all-day, *From* and *To* (each one row with date and time side by side under a single label), assigned people, recurrence, reminders, location, description. Visibility, the countdown switch, colour, icon, sync target and attachment sit behind "More settings", whose summary names them (`eventAdvancedTopics()`, built from the field labels and leaving out what the dialog does not show - visibility where nobody else reads the calendar, the attachment without Documents access). Editing opens that section only for what the event shows nowhere else: a restricted visibility, the countdown, an attachment; colour and icon are visible on the event itself. A visibility warning or a sync-target hint that appears while the section is closed opens it. The whole-series confirmation, the validation and every field id are unchanged. On a 375 x 812 phone the new-event dialog went from 1559px of content in a 523px body to 955px, at 1280 x 800 from 1149px in 608px to 839px. The "Nobody" choice in the people picker (shared with tasks, budget and schedule) is a plain chip without a placeholder avatar.
...
- **Month titles on a phone (v2.65.0, moved 2026-09-24):** below 640px a month cell reduces every entry to a dot by default. The "Event titles" toggle keeps the chips instead, as compact single-line rows, up to three per cell (`month-view--titles`; the dot rules in `public/styles/calendar.css` are scoped to `:not(.month-view--titles)`). Since 2026-09-24 it sits on the month itself, as a pressed/unpressed button in the header of the day list, instead of under Display in the filter sheet - a switch that changes the view belongs to the view. Stored per device, not per household (`yuvomi:calendar:month-titles` in `localStorage`, default off), because whether a title fits is a question of the screen, and the same person reads the same calendar on a monitor in the evening.
...
- **Overview:** one flat row per participating member — avatar, name, balance, and a progress bar
  ...
  ranking** - and since the 2026-09-23 critique neither is the dashboard widget.
...
**Deep link into a tab (2026-09-23):** `/budget?tab=split-expenses` opens Shared expenses, and `&group=<id>` selects that group - the target of the dashboard's "open balance" tile.
