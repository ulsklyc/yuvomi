...
- **Signing out other devices (#1354).** Settings → Account → "Other devices" calls
  `POST /api/v1/auth/logout-others` (CSRF, own rate limiter), which ends every session of the user
  except the calling one through `invalidateUserSessions` - the same path a password change and
  enabling 2FA take - and answers `{ ok: true, ended }`. Only a browser session may call it: an API
