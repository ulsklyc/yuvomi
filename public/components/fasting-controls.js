...
import { FASTING_PRESETS, normalizeGoalHours, fastingDisplayModel, fastingNotificationAvailability, formatFastingClock } from '/utils/health-fasting.js';
...
export function fastingPreferencesHtml(settings = {}) {
  ...
  return `<div class="fasting-preferences">
    ...
    <section class="fasting-notifications"><h3 class="u-section-title fasting-help-heading">${esc(t('health.fasting.notifications'))}${fastingHelpHtml(t('health.fasting.notifications'), [t('health.fasting.reminderHint')])}</h3>
      <label class="form-check"><input type="checkbox" data-fasting-remind-goal ${settings.remind_goal ? 'checked' : ''} ${available.goal ? '' : 'disabled'}>${esc(t('health.fasting.goalReached'))}</label>
      <label class="form-check"><input type="checkbox" data-fasting-remind-next ${settings.remind_next_start ? 'checked' : ''} ${available.next ? '' : 'disabled'}>${esc(t('health.fasting.remindNext'))}</label>
    </section>
