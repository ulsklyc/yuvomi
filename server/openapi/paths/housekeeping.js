...
const VISIT_CAPABILITY_NOTE = 'Each visit carries `can_edit` and `can_delete`: true when the caller may write to the Housekeeping module and the visit is either unpaid or the caller is an admin '
  ...
  + '`receipt_document_name` is set only when the caller may read that document: Documents module access, for API tokens a `documents:read` scope, '
  + 'and the document\'s own visibility. Otherwise it is `null` while `receipt_document_id` stays, so a visit keeps its receipt link.';
