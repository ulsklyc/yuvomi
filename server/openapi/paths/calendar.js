...
const ATTACHMENT_RIGHTS = ' A new attachment creates a document in the Documents module and therefore needs write access '
  ...
  + 'document further only for a caller with write access to documents who can see it and may manage it (its creator - for an attachment the event creator - or an admin), otherwise it only narrows. The default-assignee sync of connected calendars adds the new assignee only when the event is visible to its assignees and the document is already shared with selected members; on a private event or one visible to everybody it adds nobody. It never makes a document visible to the family and never opens a private one. A copy made on split or detach belongs to the owner of its source document. A split or a '
