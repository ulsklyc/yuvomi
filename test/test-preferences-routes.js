...
test('PUT disabled_modules changes Health and fasting cleanup atomically', async () => {
  ...
  db.exec(`CREATE TRIGGER test_fail_preferences_fasting_delete
    ...
    END`);
  try {
    const failed = await put({ disabled_modules: ['health'] }, { role: 'admin' });
    assert.equal(failed.status, 500);
    assert.deepEqual(JSON.parse(db.prepare("SELECT value FROM sync_config WHERE key = 'disabled_modules'").get().value), []);
    assert.equal(db.prepare("SELECT count(*) AS count FROM reminders WHERE entity_type = 'fasting_goal' AND created_by = ?").get(userId).count, 1);
  } finally {
    db.exec('DROP TRIGGER IF EXISTS test_fail_preferences_fasting_delete');
  }

  const disabled = await put({ disabled_modules: ['health'] }, { role: 'admin' });
