...
> **Externer Dokumentenspeicher braucht ein eigenes Backup.** Datenbank-Backups enthalten Metadaten und Verweise, nicht die Binärdateien in einem lokalen Ordner, auf WebDAV oder in Google Drive. Sichere das gewählte Ziel separat. Yuvomis Sichtbarkeitsregeln beschränken weder den verbundenen Drive-Eigentümer noch Personen mit Zugriff auf den Ordner `Yuvomi/Documents`. WebDAV-Ziele aus der Admin-Oberfläche müssen zu öffentlichen Adressen auflösen; für ein vertrauenswürdiges LAN- oder Loopback-Ziel setze `DOCUMENT_STORAGE_WEBDAV_URL` über die Deployment-Umgebung oder `DOCUMENT_STORAGE_WEBDAV_ALLOW_PRIVATE_NETWORK=true`, um private Ziele auch aus der UI zuzulassen.
